...
export class SelectRoot extends HTMLElement {
  ...
  #handleClick = (event) => {
    ...
    const trigger = /** @type {HTMLElement} */ (event.target).closest("select-trigger");
    if (trigger) {
      this.#isKeyboardOpen = false;
   ...
   */
  #handleFocusIn = (event) => {
    const item = /** @type {HTMLElement} */ (event.target).closest("select-item");
    if (item) {
  ...
  #navigateItems(direction) {
    const items = Array.from(this.querySelectorAll("select-item:not([disabled])"));
  ...
  #setupClickOutside() {
    this.#clickOutsideHandler = (event) => {
      if (!this.contains(/** @type {Node} */ (event.target)) && this.hasAttribute("open")) {
        this.removeAttribute("open");
      }
    };
